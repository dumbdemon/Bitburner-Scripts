...
export async function main(ns) {
  ...
  switch (whatDo) {
    case "buy":
      ...
      if (!isFinite(reqMny)) {
        ns.tprintf(
          `The RAM you passed has resulted in INFINITE required money. Obviously, you don't have that!`
        );
        ns.exit();
      ...
      if (reqMny > 0) {
        ...
        if (ns.getPurchasedServers().length == startCnt)
          ns.tprintf(
            `No servers purchased!\nYou need at least ${ns.nFormat(
              oneSrvCost,
              "$0.000a"
            )} for one server.`
          );
          ...
          ns.tprintf(
            `You need at least ${ns.nFormat(reqMny, "$0.000a")} to get ${
              amtMore == 1 ? "one" : amtMore
            } more server${amtMore == 1 ? "!" : "s!"}`
          );
      }
      break;
    case "query":
      if (ns.getPurchasedServers().length == ns.getPurchasedServerLimit()) {
        ...
        if (isFinite(queryMny)) {
          ns.tprintf(
            `However, you will need ${ns.nFormat(
              queryMny,
              "$0.000a"
            )} to buy ${ns.getPurchasedServerLimit()} servers with ${ns.nFormat(
              RAM * 1e6,
              "0.00b"
            )} of RAM\nFor at least one, you need ${ns.nFormat(
              oneSrvCost,
              "$0.000a"
            )}.`
          );
          ...
          ns.tprintf(
            `However, the RAM query you passed has resulted in INFINITE required money. Obviously, you don't have that!`
          );
        }
      } else if (isFinite(reqMny)) {
        queryMny -= ns.getPurchasedServerCost(RAM) * startCnt;
        ns.tprintf(
          `You will need ${ns.nFormat(queryMny, "$0.000a")} to buy ${
            ns.getPurchasedServerLimit() - startCnt
          } servers with ${ns.nFormat(
            RAM * 1e6,
            "0.00b"
          )} of RAM.\nFor at least one, you need ${ns.nFormat(
            oneSrvCost,
        ...
        ns.tprintf(
          `The RAM query you passed has resulted in INFINITE required money. Obviously, you don't have that!`
        );
      }
      break;
    default:
      ns.tprintf(
        `Type \`run buyServers.js buy <RAM>\` to buy servers, or type \`run buyServers.js query <RAM>\` to get an estimate.\nIf no RAM is passed, the script will assume 8GB of RAM.`
      );
  }
}
