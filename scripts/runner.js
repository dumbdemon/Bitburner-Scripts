...
export async function main(ns) {
  ...
  for (let trgt of trgts) {
    ...
    let freeRAM =
      (ns.getServerMaxRam(source) - ns.getServerUsedRam(source)) * 1e6;
    ...
    if (myHckLvl >= srvhckLvl) {
      if (!ns.getRunningScript(hlp.hacker, source, trgt)) {
        if (source == "home" || source.startsWith(hlp.buySrvName)) {
          try {
            ...
            if (freeRAM > ns.getScriptRam(hlp.hacker, source) * threads) {
              ...
              ns.print(
                `\nRunning "${
                  hlp.hacker
                }" targeting [${trgt.toUpperCase()}] with ${threads} threads!`
              );
            } else {
              ns.print(
                `\nCan't run "${
                  hlp.hacker
                }" targeting [${trgt.toUpperCase()}]!\nNot enough free RAM!\nNeeded ${ns.nFormat(
                  hackerRAM,
                  "0.00b"
                )}/Have ${ns.nFormat(freeRAM, "0.00b")}`
              );
            ...
            if (freeRAM > ns.getScriptRam(hlp.hacker, source)) {
              ...
              ns.print(
                `Running "${hlp.hacker}" targeting [${trgt.toUpperCase()}]!`
              );
            } else {
              ns.print(
                `\nCan't run "${
          ...
          if (freeRAM > ns.getScriptRam(hlp.hacker, source)) {
            ...
            ns.print(
              `\nRunning "${
                hlp.hacker
              }" on [${source.toUpperCase()}] targeting [${trgt.toUpperCase()}]!`
            );
          } else {
            ns.print(
        ...
        ns.print(
          `\n"${
            hlp.hacker
          }" is already targeting [${trgt.toUpperCase()}] on [${source.toUpperCase()}]!`
        );
    } else
      ns.print(
        `\nCan't run "${
