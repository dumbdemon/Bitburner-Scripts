...
export async function main(ns) {
  ...
  var ramMIN =
    Math.ceil(
      ns.getScriptRam(hlp.runner, "home") + ns.getScriptRam(hlp.hacker, "home")
    ) * 1e9;
  ns.print(
    `I need at least ${ns.nFormat(
      ramMIN,
      "0.00b"
    )} of RAM on all targetable servers! Checking if "${hlp.runner}" can run...`
  );
  ...
  for (let trgt of recievers) {
    if (ns.hasRootAccess(trgt)) {
      if (ns.getServerMaxRam(trgt) > ramMIN) {
        ns.print(
          `Killing all scipts using "${hlp.hacker}" on [${trgt.toUpperCase()}]!`
        );
        ...
        ns.print(
          `\u00bb "${hlp.runner}" has started on [${trgt.toUpperCase()}]!`
        );
      } else {
        ns.print(
          `Unable to run on [${trgt.toUpperCase()}]!\n\u00bb Not enough RAM!`
        );
      }
